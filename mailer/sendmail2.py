...
def get_conf(filepath=""):
    ...
    for line in lines:
        ...
        cfgs[key] = str(value).strip(" ").strip("\n")
    return cfgs
...
def main():
    ...
    parser.add_argument("-f", '--file', help='.mailerrc配置文件的位置')
    ...
    port = int(cfgs.get("port", 25))
    password = getpass.getpass(prompt="请输入邮箱【{}】的发送密码: ".format(sender))
...
if __name__ == "__main__":
    main()
    # cfgs = get_conf(filepath="c:/Users/biao/Desktop/rubbish/code/python/mailerrc")
    # print(cfgs)
